...
public class Parallax : MonoBehaviour
{
    ...
    [SerializeField, HideInInspector] private bool useTilePooling = true;
    [SerializeField, HideInInspector] private bool forceFillVisibleGaps = true;
    [SerializeField, Range(0f, 0.25f), HideInInspector] private float tileOverlapX = 0.02f;
    [SerializeField, Range(0f, 0.25f), HideInInspector] private float tileOverlapY = 0.02f;
    [SerializeField, Range(0f, 2f), HideInInspector] private float coveragePaddingTiles = 0.2f;
    [SerializeField, Range(0f, 4f), HideInInspector] private float cullPaddingTiles = 1f;
    [SerializeField, Min(0), HideInInspector] private int prewarmTilesX = 2;
    [SerializeField, Min(0), HideInInspector] private int prewarmTilesY;
    [SerializeField, Min(1), HideInInspector] private int maxCreatePerFramePerLayer = 8;
    [SerializeField, Min(0), HideInInspector] private int maxPooledTilesPerLayer = 12;
    ...
    private void LateUpdate()
    {
        ...
        for (int i = 0; i < layers.Length; i++)
        {
            ...
            if (useInfinite) UpdateInfiniteLayer(layer, camDelta);
            else UpdateSingleLayer(layer, camDelta);
        }
    }
    ...
    private void EnsureCoverage(ParallaxLayer layer, Vector3 camDelta, ref int createdThisFrame)
    {
        ...
        float baseCenterX = layer.startX + (camDelta.x * layer.factorX);
    ...
    private void UpdateTilePositions(ParallaxLayer layer, Vector3 camDelta)
    {
        ...
        for (int i = 0; i < layer.keyBuffer.Count; i++)
        {
            ...
            tile.position = new Vector3(GetTileCenterX(layer, key.x, camDelta), GetTileCenterY(layer, key.y, camDelta), layer.startZ);
        }
    }
    ...
    private void CreateTile(ParallaxLayer layer, int indexX, int indexY)
    {
        ...
        Transform clone = GetTileFromPool(layer) ?? Instantiate(layer.layer, layer.layer.parent);
    ...
    private void RecalculateBounds(ParallaxLayer layer)
    {
        ...
        {
            layer.tiles[new Vector2Int(0, 0)] = layer.layer;
            layer.minX = layer.maxX = layer.minY = layer.maxY = 0;
            return;
        }
